...
const getBoostedCount = async (boss) => {
	const bossesPerCount = new Map(Object.entries(await readJsonFile('./data/boost-counts.json')));
	for (const [count, bosses] of bossesPerCount) {
		if (bosses.includes(boss)) return Number(count);
	}
};
...
${boostedToday} is boosted for the ${formatOrdinals(timesBoosted)} time today.

${newlyPossibleTomorrow} can be boosted again starting tomorrow, as it’s been 30 days since it was last boosted.
`.trim();
console.log(message);
